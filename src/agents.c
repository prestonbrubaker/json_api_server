...
void generate_agents(Agent agents[]) {
    for (int i = 0; i < AGENT_COUNT; i++) {
        ...
        for (int j = 0; j < 10; j++) {
            agents[i].genes[j] = (float)(rand() % 1000) / 1000.0f;
        }
    }
}
...
void update_agents(Agent agents[]) {
    for (int i = 0; i < AGENT_COUNT; i++) {
        ...
        if ((float)(rand() % 1000) / 1000.0f < pow((double)agents[i].genes[0], 3.0)) {
            agents[i].x += 0.3f * ((float)(rand() % 10000) / 10000.0f - 0.5f);
        }

        if ((float)(rand() % 1000) / 1000.0f < pow((double)agents[i].genes[1], 3.0)) {
        ...
        if (rand() % 100 < 20 && agents[i].food > 0) {
            agents[i].food--;
        }
        if (rand() % 2 == 0) {
    ...
    if (rand() % 100 == 0) {
        ...
        agents[lowest_food_index].food = agents[j].food;
    }
